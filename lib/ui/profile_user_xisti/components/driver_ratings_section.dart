...
class DriverRatingsSection extends StatelessWidget {
  ...
  const DriverRatingsSection({super.key, required this.driverRatings});
          ...
          Row(
            children: [
              SizedBox(
                width: 100,
                height: 40,
                child: Stack(children: _buildOverlappingAvatars()),
              ),
              const SizedBox(width: 8),
              Container(
  ...
  List<Widget> _buildOverlappingAvatars() {
    ...
    for (
      int i = 0;
      i < (driverRatings.length > 3 ? 3 : driverRatings.length);
      i++
    ) {
      avatars.add(
        Positioned(
